...
app.post('/api/user/address', authMiddleware, (req, res) => {
  const { name, phone, line1, line2, city, state, zip, country } = req.body;
  if (!name || !phone || !line1 || !city || !state || !zip || !country) {
    return res.status(400).json({ error: 'Missing address fields' });
  }
  const address = { id: `addr-${Date.now()}`, name, phone, line1, line2, city, state, zip, country, isDefault: req.user.addresses.length === 0 };
